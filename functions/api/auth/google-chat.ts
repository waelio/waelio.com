/**
 * POST /api/auth/google-chat
 * Google sign-in for waelio.com/chat only.
 * Uses chat_session cookie — NOT the private session cookie.
 * Any verified Google account may sign in (unlike /api/auth/google).
 */
import {
...
export const onRequest: PagesFunction<CFContext["env"]> = async (context) => {
    ...
    try {
        ...
        return json(200, payload, {
            "Set-Cookie": createChatSessionCookie(session),
        });
    } catch (error) {
