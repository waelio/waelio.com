...
const closestPlugin: ComputedRef<string | boolean> = computed(() => {
  return contain.value.find((one) =>
    one.name ? one.name.toUpperCase().match(props.name.toUpperCase()) : false
  );
});
const notFound = "Who?";
const pluginName = computed(() => {
  return closestPlugin.value ? closestPlugin.value.name : notFound;
});
...
const dec2 = ref(
  "2c550312040321161b021246555b550312040321161b021245555b550312040321161b021244552a"
);
const dec3 = ref("2c465b455b445b435b425b415b405b4f5b4e2a");
const dec4 = ref(
  ""
);
  ...
  <div class="util-page">
    ...
    <div v-if="!hidParams" class="params-selection">
      <div class="param param1">
        ...
        <select
          id="param-one-select"
          v-model="param1"
          :class="{ 'bg-dark-200 text-blue-300': isDark }"
        >
    ...
    <div v-if="pluginName.value !== notFound">
      <p class="my-4 text-left px-4 text-lg">
        Using function name
        <code lang="js" class="text-blue-700 text-bold">{{
          closestPlugin.name || notFound
        }}</code
        >.
      </p>

      <p class="my-4 text-left px-4 text-lg">The following output is the result of:</p>
      <div v-if="param2" class="result_pan mx-auto">
        <code
          class="block mx-auto font-mono p-1 rounded text-shadow-lg text-x0"
          :class="{ 'bg-light-blue-200': !isDark }"
          lang="javascript"
        >
          waelioUtils.{{ pluginName }}({{ param1 }}, {{ param2 }})</code
        >
        <pre
          class="mx-auto my-8 bg-dark-100 p-1 overflow-y-auto full-height rounded-sm text-white"
          >{{ waelioUtils[pluginName](param1, param2) }}</pre
        >
        ...
        <code
          ...
          lang="javascript"
        >
          waelioUtils.{{ reversePlugin }} ( {{ param1 }}, {{ param2 }} )</code
        >
        <pre
          v-if="isSpecial"
          class="mx-auto my-8 bg-dark-100 p-1 overflow-y-auto full-height rounded-sm text-white"
          >{{ reverseResult || waelioUtils[reversePlugin](param1, param2) }}</pre
        >
      </div>
      <div v-else class="result_pan mx-auto">
        <code
          ...
          lang="javascript"
        >
          waelioUtils.{{ pluginName }}({{ param1 }})</code
        >
        <pre
          v-if="pluginName !== 'generateId'"
          class="mx-auto my-8 bg-dark-100 p-1 overflow-y-auto full-height rounded-sm text-white"
          >{{ waelioUtils[pluginName](param1) }}</pre
        >
        <div
          v-else
          class="mx-auto my-8 bg-dark-100 p-1 overflow-y-auto full-height rounded-sm text-white"
