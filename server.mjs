...
function loadUsers() {
  const users = new Map();
  for (let i = 1; i <= 2; i++) {
    const raw = process.env[`AUTH_USER_${i}`];
    if (!raw) continue;
    const sep = raw.indexOf(':');
    if (sep === -1) continue;
    users.set(raw.slice(0, sep).toLowerCase(), raw.slice(sep + 1));
  }
  return users;
}
...
function verifyToken(token) {
  ...
  if (Number.isNaN(expiry) || expiry < Date.now() / 1000) return null;
...
const server = createServer(async (req, res) => {
  ...
  if (path === '/api/login' && req.method === 'POST') {
    ...
    if (!storedHash || storedHash !== hashPassword(body.password)) {
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`Auth users loaded: ${USERS.size}`);
});
