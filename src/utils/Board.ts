...
if (typeof console === 'undefined' || !console) var console = { log(msg) { alert(msg) } }
...
RICKO.SudokuViewCanvas = function(containerId, parentId, instructionsId, boardsId) {
  ...
  const body = getEl(parentId)
  const inst = getEl(instructionsId)
  const container = getEl(containerId)
  const boards = getEl(boardsId)
  // Canvas elements
  let board = null
  let grid = null
  let light = null
  ...
  function setBoards(boardNames, onClick) {
    ...
    const clickFunc = function(event) {
      ...
      const boardName = unescape(event.target.hash.substring(1, this.href.length))
      onClick(boardName)
      return false
    }
  ...
  function resizeBoard() {
    const bh = body.innerHeight || body.clientHeight || body.offsetHeight || body.scrollHeight
    const bw = body.innerWidth || body.clientWidth || body.offsetWidth || body.scrollWidth
    let w = Math.floor(((bw > bh) ? bh : bw) - (pad * 2))
    let iw = bw - w - (pad * 3.5)
    if (iw < 200) {
      w -= (200 - iw)
      iw = 200
  ...
  function clickBoard(event) {
    ...
    const target = event.target || event.toElement || event.srcElement
