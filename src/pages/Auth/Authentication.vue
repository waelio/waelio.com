<template>
    ...
    <div class="full-width column no-wrap justify-evenly items-stretch content-between">
      <div class="text-center q-py-sm q-my-sm bg-red text-white">This site is under continues developments!<q-spinner-gears class="q-mx-sm" size="2rem" color="primary" /></div>
    <div v-if="authState !== 'signedin'" class="text-center q-py-sm bg-warning text-white">You are signed out.</div>
    <div v-else class="text-center q-py-sm bg-green text-white">Hello, {{user.username}}</div>
      <amplify-authenticator class="aws-auth second">
        <amplify-sign-out></amplify-sign-out>
      </amplify-authenticator>
    </div>
      <q-separator inset spaced size="20" />
...
export default {
  ...
  data () {
    return {
      ...
      tempData: null,
      formsStates: ['SignIn', 'SignUp', 'ForgotPassword', 'ChangePassword', 'VerifyEmail'],
      authState: 'SignIn',
      user: null,
      isLoading: false,
      customState: null,
  ...
  methods: {
    oopenURL: async (url) => {
      try {
        const result = await Auth.AuthSession.startAsync({ authUrl: this.buildUrl })
    ...
    getTokenbyCode: async (code) => {
      ...
      try {
        const res = await fetch(
          'https://auth.waelio.com/oauth2/token',
          {
            method: 'POST',
            headers: {
