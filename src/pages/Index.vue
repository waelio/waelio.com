<template>
  <div class="q-pa-0" :style="{'margin-top':'-34px'}">
    <q-timeline dark layout="loose" class="bg-grey-9 q-pa-xs">
      ...
      >
        <template v-slot:title>
          <q-btn flat>
            <p
              class="text-h3 text-weight-this w-100"
            ...
            <q-tooltip
              ...
              self="center middle"
              :content-class="`bg-${item.color}-light-2`"
              :delay="1000"
              :hide-delay="2000"
            >
              <iframe
                ref="timeline_iframe"
                class="rounded no-border q-pa-0"
                :style="{ width: '75vh', height: '50vw' }"
                :src="item.link"
                frameborder="0"
              ></iframe>
            </q-tooltip>
        ...
        <template v-slot:subtitle>
          <p class="text-h4 font-weight-bold" :class="`text-${item.color}`">
            {{ item.year }}
          </p>
        </template>
        <p class="text-h6 text-weight-bold font-italic w-100 text-justify">
          {{ item.text }}
        </p>
      </q-timeline-entry>
    </q-timeline>
    ...
    return {
      history: [
        ...
        },
        {
          ...
          year: 'Today...',
          text:
            'Pretty much daily, I perform at least one of following: reading an article, attending an online siminar, watching a youtube lesson or simply refactoring one of my projects.'
        }
      ]
    }
  }
